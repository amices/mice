...
* Adds a new function `mice.impute.lasso.pmm()` for imputation of high-dimensional data by means of the lasso. The function is based on the `glmnet` package and uses the `matchindex` algorithm for imputation. The function is designed for high-dimensional data sets with many variables and few observations. The function is a drop-in replacement for `mice.impute.pmm()` and can be used in the `mice()` function by specifying `method = "lasso.pmm"`. Specify `eps = 0` to inactive the old but slow `remove.lindep()` function, and let lasso do the work.
