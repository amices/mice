...
test_that("Returns requested length", {
  expect_equal(length(mice.impute.pmmsplit(y, ry, x)), sum(!ry))
  expect_equal(length(mice.impute.pmmsplit(y, ry, x, wy = wy1)), sum(wy1))
  expect_equal(length(mice.impute.pmmsplit(y, ry, x, wy = wy2)), sum(wy2))
  expect_equal(length(mice.impute.pmmsplit(y, ry, x, wy = wy3)), sum(wy3))
  expect_equal(length(mice.impute.pmmsplit(y, ry, x, wy = wy4)), sum(wy4))
})

test_that("Excludes donors", {
  expect_false(all(c(15:25) %in% mice.impute.pmmsplit(y, ry, x, exclude = c(15:25))))
})

imp1 <- mice(nhanes, printFlag = FALSE, seed = 123)
imp2 <- mice(nhanes, printFlag = FALSE, seed = 123, exclude = c(-1, 1032))
test_that("excluding unobserved values does not impact pmmsplit", {
...
test_that("cancor proceeds normally", {
  expect_silent(imp1 <- mice(data1, method = "pmmsplit", remove.collinear = FALSE, eps = 0,
...
test_that("cancor proceeds normally with many duplicates", {
  expect_warning(imp2 <- mice(data2, method = "pmmsplit", remove.collinear = FALSE, eps = 0,
...
test_that("cancor with many junk variables does not crash", {
  expect_warning(imp3 <- mice(data3, method = "pmmsplit", remove.collinear = FALSE, eps = 0,
                              maxit = 1, m = 1, seed = 1, print = FALSE))
})
