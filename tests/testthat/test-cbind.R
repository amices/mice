test_that("CBIND-001: combines imputations", {
  data <- nhanes
...
test_that("CBIND-002: combines imputations with blocks", {
  data <- nhanes
  data1 <- data[, c("age", "bmi")]
  data2 <- data[, c("hyp", "chl")]
...
test_that("CBIND-003: duplicate variable adds a column", {
...
test_that("CBIND-004: duplicate blocks names renames block", {
...
test_that("CBIND-005: returns imp1 object if there is nothing to bind", {
...
test_that("CBIND-006: replicates unnamed constant", {
...
test_that("CBIND-007: appends names vectors and constants", {
...
test_that("CBIND-008: appends matrix, factor and data.frame", {
...
test_that("CBIND-009: appends matrix, factor and data.frame (imp9)", {
  expect_identical(ncol(complete(imp9)), 9L)
})

impc <- mice.mids(imp9, max = 2, print = FALSE)
test_that("CBIND-010: combined object works as input to mice.mids", {
  expect_true(is.mids(impc))
})

test_that("CBIND-011: cbind does not throw a warning (#114)", {
