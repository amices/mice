...
test_that("Case C finds predictorMatrix", {
  ...
  expect_identical(dim(imp3$predictorMatrix), c(4L, 4L))
})
...
# save for comparsin with case D
imp1_blocks <- imp1
imp2_blocks <- imp2
imp3_blocks <- imp3
...
imp1 <- mice(data,
             formulas = form1, print = FALSE, m = 1, maxit = 1, seed = 11
)

# same model using dot notation
form2 <- list(bmi ~ ., hyp ~ ., chl ~ .)
imp2 <- mice(data,
             formulas = form2, print = FALSE, m = 1, maxit = 1, seed = 11
)
...
imp3 <- mice(data,
             formulas = form3, print = FALSE, m = 1, maxit = 1, seed = 11
)

# same model using dot notation
form4 <- list(bmi + hyp ~ ., chl ~ .)
imp4 <- mice(data,
             formulas = form4, print = FALSE, m = 1, maxit = 1, seed = 11
)
...
# replicate models used in case C, but now specified with formulas
imp1 <- mice(data, formulas = list(bmi ~ ., chl ~ ., hyp ~ .), print = FALSE, m = 1, maxit = 1, seed = 11)
imp2 <- mice(data, formulas = list(bmi + chl ~ ., hyp ~ .), print = FALSE, m = 1, maxit = 1, seed = 11)
imp3 <- mice(data, formulas = list(bmi + chl + hyp ~ .), print = FALSE, m = 1, maxit = 1, seed = 11)

test_that("Case C and D yields same imputations", {
  expect_equal(complete(imp1), complete(imp1_blocks))
  expect_equal(complete(imp2), complete(imp2_blocks))
  expect_equal(complete(imp3), complete(imp3_blocks))
})
...
# DEPRECATED - ONLY SQUARE ALLOWED
