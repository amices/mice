...
test_that("tasks work with factor with same number of categories", {
  ...
  expect_error(imp2 <- mice(nhanes2, m = 3, maxit = 1, task = "fill", methode = "pmm", models = imp1$models, print = FALSE), "Number of imputations")
  expect_silent(imp2 <- mice(nhanes2, m = 2, maxit = 1, task = "fill", methode = "pmm", models = imp1$models, print = FALSE))
