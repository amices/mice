...
traindats <- lapply(impdats, function(dat) {
  subset(dat, set == "train", select = -set)
})
testdats <- lapply(impdats, function(dat) {
  subset(dat, set == "test", select = -c(set))
})
