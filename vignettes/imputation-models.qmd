...
> This vignette describes a new feature in the `mice` package that is currently under development. The functionality may change in future releases, and the examples may not work as expected. Use at your own risk.
...
You can automate these checks using `scan.newdata()` and align new data structures using `coerce.newdata()` or `make.newdata()`.

### Using `scan.newdata()`, `coerce.newdata()`, and `make.newdata()`
...
scan.newdata(newdata, trained$models)
```
...
#### Fix the Data with `coerce.newdata()`

You can fix detected mismatches by coercing the data to the expected format:

```{r}
newdata_fixed <- coerce.newdata(newdata, trained$models)

# Check again
scan.newdata(newdata_fixed, trained$models)
```

Now all variables match in type and structure, and `can_fill` for `bmi` is changed to `TRUE`.

#### Start from Scratch with `make.newdata()`

If you don’t have any data yet, or want a fully clean starting point, use `make.newdata()`:

```{r}
# Create a new empty row with correct types and levels
template <- make.newdata(models = trained$models)
...
| `calltype`   | Form of imputation model                     | I | `character`       | YES | YES | NO  |
