...
sampler.dt <- function(data, m, ignore, where, imp, blocks, method,
    ...
    for (h in visitSequence) {
      ...
      if (univ) {
        for (j in b) {
          ...
          # imp[[j]] is updated by sampler.univ.dt
      ...
      #   for (j in b) {
      #     n <- nrow(imp[[j]])
      #     rows_in_long <- rep(imp[[j]][["row_id"]], m) + (0:(m - 1L)) * nrow(data)
      #     val <- model.frame(as.formula(theMethod), long[rows_in_long, ],
      #                        na.action = na.pass)[[1L]]
      #     for (i in seq_len(m)) {
      #       value <- val[((i - 1L) * n + 1L):(i * n)]
      #       set(imp[[j]], j = i, value = value)
      #     }
      #     impute.long(long = long, imp = imp, where = where, vars = j, r = r)
      #   }
      # }
...
sampler.univ.dt <- function(data, r, where, pred, formula, method, j, k,
