...
#' \code{mice()} sets
#' \code{calltypes = "formula"} for any block
#' for which a formula is specified. Otherwise, \code{calltypes = "pred"}.
...
mice <- function(data,
  ...
  if (mp & mb & mf) {
    ...
    calltypes <- make.calltypes(calltypes, predictorMatrix, formulas, "pred")
  }
