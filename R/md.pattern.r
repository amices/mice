...
#'
#'This function is useful for visually inspecting the structure of missing
#'observations in the data.
#'
#'@param x An object of class \code{mdpat}, produced by \code{md.pattern()}
#'@return A rectangle plot of the missing data pattern, where the numerical pattern and 
#'missingness information is contained in the margins.
#'@author Gerko Vink, 2018
#'@seealso \code{\link{md.pattern}}
#'@method plot md.pattern
#'@export
plot.mdpat <- function(x){
  plot.new()
  nmis <- r[dim(x)[1], 1:dim(x)[2] - 1]
  if (all(tail(x, n=1) == 0)){
    R <- t(as.matrix(x[1:nrow(x)-1, 1:ncol(x)-1]))
  } else {
  ...
  rect(M[, 2], M[, 1], M[, 2] + 1, M[, 1] + 1, col=shade)
  for(i in 1:ncol(R)){
    text(i - .5, nrow(R) + .3, colnames(x)[i], adj = c(0.5,0))
    text(i - .5, -.3, nmis[i])
  }
  for(i in 1:nrow(R)){
