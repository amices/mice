...
estimice <- function(x, y, ls.meth = "qr", ridge = 1e-05, ...) {
  ...
  if (ls.meth == "qr") {
    ...
    if (inherits(v, "try-error")) {
      ...
      mess <- "mice detected that your data are (nearly) multi-collinear. It applied a ridge penalty to continue calculations, but the results can be unstable. Does your dataset contain duplicates, linear transformation, or factors with unique respondent names?"
