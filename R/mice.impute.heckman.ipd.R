...
mice.impute.heckman.ipd <-
  function(y,ry,x,wy = NULL, type, family, full = FALSE, pmm = FALSE, meta_method,...) {
    ...
    sel_name    <- colnames(x)[type == -3] # names of variables in selection alone
    out_name    <- colnames(x)[type == -4] # names of variables in outcome alone
    ...
    if (Marg_ind == 1) {
      ...
      Marg_ind  <- ifelse(length(fit_list[studytype == 1]) >= 2, 1, 0) # Calculate marginal distribution if there are parameters for at least 2 studies
    }
    ...
    if (Marg_ind == 1) {
      ...
      coef_mat_s <- as.matrix(Reduce(rbind,     # this is the same as rbindlist but without calling data.table package
...
copulaIPD <- function(data, sel, out, family, send) {
  family<-ifelse(family=="probit","probit","N")
  ...
  if (fit_ind != 1) {
    fit <- try(gam::gam(data = data, formula = out,
                        family = ifelse(family == "probit","binomial","gaussian")),
               silent = TRUE)
