...
#' 1: Predictor in the outcome model,-2: Cluster id (study id),
...
#' @param family family of the missing variable {"gaussian","probit"}
...
#' "ml" (maximum likelihood), "reml" (restricted maximul likelihood) or "mm"
#' method of moments.
...
mice.impute.heckman.ipd <-
  function(y,
    ...
    # Rename covariates
    colnames(x) <-
      paste0("x_", 1:length(colnames(x))) #change the covariates name for avoiding conflicts when y is covariate
    excl_name <-
      colnames(x)[type == -3] #names of exclusion variables
    cov_name <- colnames(x)[type == 1] #names of exclusion variables
    ...
    order <- c(cov_name, excl_name)
    send <-
      length(cov_name) + length(excl_name) + 1 # Index where selection equation ends in coefficient vector
    oend <-
      send + length(cov_name) + 1  # Index where outcome equation ends in coefficient vector
    
    # Define outcome and selection equation
    out <-
      as.formula(paste0("y", "~", paste(cov_name, collapse = "+")))
    sel <-
    ...
    X <- data.frame(cbind(rep(1, nrow(x)), x[, order]))
    
    # 2. Step 1: Get the theta estimators for each study ----
    
    if (Marg_ind == 1) {
      df_list <-
        split.data.frame(data, x[, group_name]) # Separate dataframe by group
      res_total <-
        suppressWarnings(
          lapply(
            df_list,
        ...
        ) #Calculate parameters for each study
      studytype <-
        sapply(res_total, function(x)
          x[[2]]) # Specify missing pattern in studies
      sporadical <-
        studytype[studytype != 0] # List sporadical missing studies
      fit_list <-
        lapply(res_total, function(x)
          x[[1]]) # Get the parameter estimates
      Marg_ind <-
        ifelse(length(fit_list[studytype == 1]) > 2, 1, 0) # Calculate marginal distribution if there are parameters for at least 2 studies
    }
    
    # 3. Step 2: Get marginal theta and var(theta) ----
    if (Marg_ind == 1) {
      coef_list <-
        lapply(fit_list[studytype != 0], `[[`, c('coefficients')) # Get effect vector for each study
      Vb_list <-
        lapply(fit_list[studytype != 0], `[[`, c('Vb')) # Get covariance matrix for each study
      #coef_mat_s <- as.matrix(rbindlist(lapply(coef_list[sporadical==1], function(x) as.data.frame(t(x))),fill=TRUE)) # Transform effects vector in a matrix with studies in rows
      dcoef_list <-
        lapply(coef_list[sporadical == 1], function(x)
          as.data.frame(t(x)))
      coef_mat_s <-
        as.matrix(Reduce(rbind, Map(function(x) {
          x[, setdiff(unique(unlist(lapply(
            dcoef_list, colnames
          ))), names(x))] <- NA
          return(x)
        }, dcoef_list)))
      ...
      outnam <-
        varnam[!varnam %in% c(selnam, "sigma.star", "theta.star")]
      ...
      if (full == TRUE) {
        total_mar <-
          draw_theta_psi_mar(
            coef_mat_s = coef_mat_s,
            Vb_list = Vb_list[sporadical == 1],
            meta_method = meta_method,
            Marg_ind = Marg_ind
          )
        Marg_ind <- total_mar[[3]]
        if (Marg_ind == 1) {
          beta_s_mar <-
            list(total_mar[[1]][selnam], total_mar[[2]][selnam, selnam])
          beta_o_mar <-
            list(total_mar[[1]][outnam], total_mar[[2]][outnam, outnam])
          rho_t_mar <-
            list(total_mar[[1]]["theta.star"], total_mar[[2]]["theta.star", "theta.star"])
          if (family != "probit") {
            sigma_t_mar <-
              list(total_mar[[1]]["sigma.star"], total_mar[[2]]["sigma.star", "sigma.star"])
          }
        }
        ...
        rho_t_mar <-
          draw_theta_psi_mar(
            ...
            vnames = "theta.star",
            meta_method = meta_method,
            Marg_ind = beta_o_mar[[3]]
          )  #copula package calls atanh(rho) as theta.star
        ...
        if (family != "probit") {
          sigma_t_mar <-
            draw_theta_psi_mar(
              ...
              meta_method = meta_method,
              Marg_ind = rho_t_mar[[3]]
            ) #copula package calls log(sigma) as sigma.star
    ...
    if (Marg_ind == 1) {
      for (i in names.clust) {
        ...
        } else if (studytype[[i]] == 2) {
          # MAR estimable
          
          beta_o_star <-
            MASS::mvrnorm(n = 1,
                          mu = coef_list[[i]][outnam],
                          Sigma = Vb_list[[i]][outnam, outnam])
          if (family != "probit") {
            sigma_star <-
              exp(MASS::mvrnorm(
                n = 1,
                mu = coef_list[[i]]["sigma.star"],
                Sigma = Vb_list[[i]]["sigma.star", "sigma.star"]
              ))#copula package calls log(sigma) as sigma.star
          } else {
            sigma_star <- NA
          }
        ...
        if (nrow(Xm) != 0) {
          #Cluster with at least one missing value in outcome equation
          y.star <-
            gen_y_star(
              Xm = Xm,
              excl_name = excl_name,
              beta_s_star = beta_s_star,
    ...
    if (Marg_ind == 0) {
      #Full data Heckman model
      warning(
        "There is not enough information to impute at the study level, so the imputation is based on the total database without study specification."
      )
      res <-
        copulaIPD(
          data = data,
      ...
      if (!class(res)[[1]] == "copulaSampleSel") {
        # model is not estimable
        stop(
          "There is not enough infomation to impute the Heckman model at marginal or study level"
        )
      }
      ...
      sigma_star <-
        ifelse("sigma.star" %in% colnames(mar.star), exp(mar.star[, "sigma.star"]), NA)
...
copulaIPD <- function(data, sel, out, family, send, oend) {
  fit_ind <- 0  # No estimable
  fit <-
    try(GJRM::gjrm(
      formula = list(sel, out),
      data = data,
  ...
  if (!any(class(fit) == "try-error")) {
    # model is estimable
    convh <-
      lqmm::is.positive.definite(fit$fit$hessian) # convergence based on hessian positive definiteness
    convg <-
      max(abs(fit$fit$gradient)) < 0.01 # convergence based on abs max gradient
    
    if (convh & convg) {
      # MNAR estimable
      fit_ind <- 1
      names <-
        c(paste0(names(fit$coefficients)[1:send], "_s"), names(fit$coefficients[(send +
                                                                                   1):length(names(fit$coefficients))]))
  ...
  if (fit_ind != 1) {
    fit <-
      try(mgcv::gam(
        ...
        family = ifelse(
          family == "N",
          gaussian,
          ifelse(family == "probit", "binomial", "nb")
        )
      ),
      silent = TRUE)
    ...
    if (!any(class(fit) == "try-error")) {
      if (fit$converged) {
        fit_ind <- 2 # MAR estimable.
        fit$Vb  <- fit$Ve
        colnames(fit$Vb) <-
          rownames(fit$Vb) <- names(fit$coefficients)
        
        if (family != "probit") {
          fit$coefficients["sigma.star"] <- log(stats::sigma(fit))
          fit$Vb <- cbind(fit$Vb, sigma.star = rep(0, nrow(fit$Vb)))
          fit$Vb <-
            rbind(fit$Vb, sigma.star = c(rep(0, nrow(fit$Vb)), 2 / (fit$df.residual -
                                                                      4)))
        }
      }
    }
  ...
  if (fit_ind == 0) {
    # not enough info to fit any model
    fit <- NA
    fit_ind <- 0
  }
...
draw_theta_psi_mar <-
  function(coef_mat_s,
    ...
    cov_mat_s  <-
      do.call("rbind", lapply(Vb_list, cov_mat_vector, vnames = vnames))
    
    # Apply multivariate random-effects meta-analysis
    
    if (Marg_ind == 1) {
      mvma <-
        try(mixmeta::mixmeta(coef_mat_s,
                             cov_mat_s,
                             method = meta_method,
      ...
      if (class(mvma) == "try-error") {
        # Use mm instead
        meta_method = "mm"
        #mvma <- try(mixmeta::mixmeta(coef_mat_s, cov_mat_s, method = meta_method, bscov = "diag", control = list(hessian = TRUE)), silent = TRUE)
        mvma <-
          try(mixmeta::mixmeta(coef_mat_s,
                               cov_mat_s,
                               method = meta_method,
...
draw_cond_theta <-
  function(theta_mar, theta_k, var_theta_k, vnames) {
    W_m <- MASS::ginv(theta_mar[[2]])
    W_k <- MASS::ginv(var_theta_k[vnames, vnames])
    S <- MASS::ginv(W_m + W_k)
    mu <-
      S %*% (W_k %*% as.vector(theta_k[vnames]) + W_m %*% as.vector(theta_mar[[1]]))
...
gen_y_star <-
  function(Xm,
           ...
           ry) {
    XOBO <-
      data.matrix(Xm[,!colnames(Xm) %in% c(excl_name)]) %*% as.vector(beta_o_star)
    
    if (studytype != 2) {
      XSBS <- data.matrix(Xm) %*% as.vector(beta_s_star)
    }
    ...
    if (family == "N") {
      if (studytype != 2) {
        ...
        y.star <-
          XOBO + as.numeric(sigma_star) * as.numeric(rho_star) * Ratio + rnorm(nrow(XSBS), 0, sd = sigma_star)
      ...
      if (studytype != 2) {
        # Not MAR
        p.star <-
          pbivnorm::pbivnorm(as.vector(XOBO),-as.vector(XSBS),-as.numeric(rho_star)) / stats::pnorm(-XSBS)
        p.star[is.na(p.star) |
                 (is.infinite(p.star) & p.star < 0) |
                 p.star < 0.0 | p.star == "0"] <- 0.0
        p.star[p.star > 1.0 |
                 p.star == "1" |
                 (is.infinite(p.star) & p.star > 0)] <- 1.0
      } else {
        expXOBO <- exp(XOBO)
