parse.ums <- function(x, ums = NULL, umx = NULL, ...) {
  ...
  if (sum(!grepl("*", mnar0, fixed = TRUE)) == 0L)
    ...
    if (sum(!grepl("*", mnar0, fixed = TRUE)) == 1L)
      mnar0[!grepl("*", mnar0, fixed = TRUE)] <- paste(mnar0[!grepl("*", mnar0, fixed = TRUE)], "*intercept", sep = "")
  else
    if (sum(!grepl("*", mnar0, fixed = TRUE)) > 1L)
      stop("Only one intercept term allowed")
  mnar <- strsplit(mnar0, "*", fixed = TRUE)
  mnar.parm <- as.numeric(unlist(lapply(mnar, function(x) x[1L]))) #e.g. c("-3","2")
  mnar.vars <- unlist(lapply(mnar, function(x) x[2L])) #e.g. c("intercept","bmi")
