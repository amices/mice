parse.ums <- function(x, ums = NULL, umx = NULL, ...) {
  if (is.null(ums)) {
    stop("Unidentifiable model specification (ums) not found.")
  }
  if (!is.null(umx)) {
    x <- base::cbind(x, umx)
  }
  ...
  mnar.parm <- mnar.parm[c(
    which(mnar.vars == "intercept"),
    which(mnar.vars != "intercept")
  )]
  mnar.vars <- mnar.vars[c(
    which(mnar.vars == "intercept"),
    which(mnar.vars != "intercept")
  )]
  xmnar <- as.matrix(cbind(
    1,
    as.matrix(x[, mnar.vars[!mnar.vars == "intercept"]])
  ))
