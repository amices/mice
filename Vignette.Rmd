...
\begin{figure}[h!]
\begin{tikzpicture}
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, fill = red!20, minimum height = 3em] at (0,0) (start) {\small Start};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (0,-2) (data) {\small 1.Data};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (2.8,-2) (prop) {\small 2.Prop};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (2.8,0) (pat) {\small 3.Patterns};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (5.6,0) (freq) {\small 4.Freq};
\node [shape=rectangle, text centered, rounded corners, draw=black, minimum height = 3em, minimum width = 6em] at (5.6,-2) (mech) {\footnotesize 5.Mechanism};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (8.6,-2) (weights) {\small 6.Weights};
\node [shape=rectangle, text centered, rounded corners, draw=black, minimum height = 3em, minimum width = 6em] at (8.6,0) (cont) {\footnotesize 7.Continuous};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (11.6,0) (type) {\small 8.Type};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em] at (11.6,-2) (odds) {\small 9.Odds};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em, fill = red!20] at (14.4,-2) (finis1) {\small Finished};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em, fill = red!20] at (14.4,0) (finis2) {\small Finished};
\node [shape=rectangle, minimum width = 6em, text centered, rounded corners, draw=black, minimum height = 3em, fill = red!20] at (5.6,-4) (finis3) {\small Finished};
\draw [->, line width = 0.8pt] (start) -- (data); 
\draw [->, line width = 0.8pt] (data) -- (prop);
\draw [->, line width = 0.8pt] (prop) -- (pat);
\draw [->, line width = 0.8pt] (pat) -- (freq);
\draw [->, line width = 0.8pt] (freq) -- (mech);
\draw [->, line width = 0.8pt] (mech) -- (weights) node [midway, above, sloped] {\footnotesize MAR}; 
\draw [->, line width = 0.8pt] (weights) -- (cont);
\draw [->, line width = 0.8pt] (cont) -- (type)node [midway, above, sloped] {\footnotesize Yes};
\draw [->, line width = 0.8pt] (cont) -- (odds) node [midway, above, sloped] {\footnotesize No};
\draw [->, line width = 0.8pt] (type) -- (finis2); 
\draw [->, line width = 0.8pt] (odds) -- (finis1);
\draw [->, line width = 0.8pt] (mech) -- (finis3) node [midway, sloped, rotate = 90, xshift = 4ex] {\footnotesize MCAR};
\end{tikzpicture}
...
The logit functions are more thoroughly explained by Van Buuren (2012, pp. 63, 64), but a quick overview will be given here. Four MAR missingness functions are known: MARRIGHT, MARMID, MARTAIL and MARLEFT missingness. Figure \ref{fig2} shows the course of the probabilities for standardized values. 
...
data2["Type"] <- c(rep("MARRIGHT", 601), rep("MARMID", 601), rep("MARTAIL", 601), rep("MARLEFT", 601))
plot1 <- xyplot(Y ~ X1, data2, group = Type, t = 'l', 
       lty=c(1,2,3,4), 
       cex = 0.5, col = rep("black", 4),
       xlab = "Values", ylab = "Probability",
       key=list(space="top", columns=4, lines = list(lty = c(1, 2, 3, 4)), text = list(c("MARLEFT", "MARMID", "MARRIGHT", "MARTAIL"), cex = 0.5)))
