...
\description{Dataset containing height and weight data (measured, self-reported) from two studies.}
\usage{data(selfreport)}
\format{
  A data frame with 2060 rows and 15 variables:
  \describe{
	\item{\code{src}}{Study, either \code{krul} or \code{mgg} (factor)}
	\item{\code{id}}{Person identification number}
	\item{\code{pop}}{Population, all \code{NL} (factor)}
	\item{\code{age}}{Age of respondent in years}
    	\item{\code{sex}}{Sex of respondent (factor)}
	\item{\code{hm}}{Height measured (cm)}
	\item{\code{wm}}{Weight measured (kg)}
	\item{\code{hr}}{Height reported (cm)}
	\item{\code{wr}}{Weight reported (kg)}
	\item{\code{prg}}{Pregnancy (factor), all \code{Not pregnant}}
	\item{\code{edu}}{Educational level (factor)}
	\item{\code{etn}}{Ethnicity (factor)}
	\item{\code{web}}{Obtained through web survey (factor)}
	\item{\code{bm}}{BMI measured (kg/m2)}
	\item{\code{br}}{BMI reported (kg/m2)}
  }
}
\details{
This dataset combines two datasets: \code{krul} data (Krul, 2010) (1257 persons) and the \code{mgg} data (Van Keulen 2011; Van der Klauw 2011) (803 persons). The \code{krul} dataset contains height and weight (both measures and self-reported) from 1257 Dutch adults, whereas the \code{mgg} dataset contains self-reported height and weight for 803 Dutch adults. Section 7.3 in Van Buuren (2012) shows how the missing measured data can be imputed in the \code{mgg} data, so corrected prevalence estimates can be calculated. 
}


\source{
Krul, A., Daanen, H. A. M., Choi, H. (2010). Self-reported and measured weight, height and body mass index (BMI) in Italy, The Netherlands and North America. \emph{European Journal of Public Health}, \emph{21}(4), 414-419. 

Van Keulen, H.M.,, Chorus, A.M.J., Verheijden, M.W. (2011). 
\emph{Monitor Convenant Gezond Gewicht Nulmeting (determinanten van) beweeg- en eetgedrag van kinderen (4-11 jaar), jongeren (12-17 jaar) en volwassenen (18+ jaar)}. TNO/LS 2011.016. Leiden: TNO.
\url{http://www.gezondeschool.info/object_binary/o11783_2011-016-monitor-convenant-gezond-gewicht-def.pdf}

Van der Klauw, M., Van Keulen, H.M., Verheijden, M.W. (2011). 
\emph{Monitor Convenant Gezond Gewicht Beweeg- en eetgedrag van kinderen (4-11 jaar),
jongeren (12-17 jaar) en volwassenen (18+ jaar) in 2010 en 2011.}
