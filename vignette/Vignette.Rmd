...
Function `ampute` has multiple options that can be specified. Figure \ref{fig1} gives an overview of all the possiblities and also in which order these steps need to be taken. If a MCAR missingness mechanism is desired, for example, steps 6 to 9 do not need to be specified. 

The underlying method of `ampute` is based on Brand's (1999) multivariate amputation procedure. An adaptation to this method is made to make continuous amputation possible. The possibility to create MNAR missingness is an extra feature as well. Nevertheless, Figure \ref{fig1} not only shows the arguments of `ampute`, it also is a nice guideline in understanding the underlying procedure. 
...
Subsequently, the default `patterns` matrix can be changed according the user's desires. For example, the missingness patterns might be changed into:
...
As you see, the matrices are exactly similar. The reason for this is that in case of `MAR` missingness, the variables that will be made missing should not be weighted (`0` in the `weights` matrix). Those are the variables with `0` in the `patterns` matrix.

As a default, the other variables are equally weighted. In case of `MNAR` missingness, the default `weights` matrix is as follows:
...
Naturally, the idea of the `weights` matrix is to weight variables differently from each other. From now on, we will impose a `MAR` mechanism to the data. We therefore focus at the default `weights` matrix that we earlier stored under `myweights`. 
...
We will now apply the settings we specified and inspect the results in two ways: boxplots and scatterplots.   
...
In reality, one might want to generate multiple kinds of missingness mechanisms within the same data set. In order to do this, I advise to ampute one data set multiple times according the desired specifications. Thereafter, for each row of the original data set, a row can be sampled from the multiple amputed data sets. For three missingness mechanisms, 1/3 of the data set would be the result of amputation 1, 1/3 from amputation 2 and the last third from amputation 3. In total, all three amputation mechanisms will be generated in the data set. 

An example is shown below. 

```{r}
ampdata1 <- ampute(testdata, prop = 0.3, patterns = mypatterns[1:2, ], 
                   weights = myweights[1:2, ], mech = "MAR")$amp
ampdata2 <- ampute(testdata, prop = 0.3, patterns = mypatterns[3:4, ], 
                   weights = c(0, 0, 1, 3, 0, 1), mech = "MNAR")$amp
ampdata3 <- ampute(testdata, prop = 0.3, patterns = c(0, 1, 1), 
                   mech = "MCAR")$amp
